...
const RouteEngine = (() => {
  ...
  function generate(level) {
    const boss = level % 10 === 0,
      ...
      rnd = random(Math.imul(level, 2654435761) ^ 195743);
    let path = [];
    for (let r = 0; r < n; r++)
      for (let c = 0; c < n; c++) path.push(r * n + (r % 2 ? n - 1 - c : c));
    // Backbite transformations preserve a Hamiltonian path while changing its shape.
    ...
    const trim =
      level < 4 ? 2 : boss ? 0 : 1 + Math.floor(rnd() * Math.min(n, 4));
...
if (typeof document !== "undefined")
  (() => {
    const $ = (id) => document.getElementById(id),
      ...
      read = (key, f) => {
        try {
          return JSON.parse(GameSave.storage.getItem("rota." + key)) ?? f;
        } catch {
          return f;
        }
      },
      save = (key, v) => {
        try {
          GameSave.storage.setItem("rota." + key, JSON.stringify(v));
        } catch {}
      },
    ...
    const sound = {
      ...
      play(kind) {
        ...
        try {
          ...
          const c = this.ctx;
          const note = (f, delay, dur, type = "sine", vol = 0.055) => {
          ...
          if (kind === "step")
            note([330, 370, 415, 494, 554][path.length % 5], 0, 0.08, "sine");
          ...
          if (kind === "win")
            [330, 415, 494, 660].forEach((f, i) =>
              note(f, i * 0.09, 0.24, "triangle", 0.04),
            );
        } catch {}
      },
    };
