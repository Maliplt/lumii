...
function createLanguagePicker(onSelect) {
  ...
  const pauseBack = document.createElement("button");
  pauseBack.type = "button";
  pauseBack.id = "pause-language-back";
  pauseBack.hidden = true;
  document.getElementById("pause-settings").append(pauseBack);
  pauseBack.onclick = () => close(true);
  function position() {
    if (panel.hidden) return;
    if (panel.closest("#pause-settings")) {
  ...
  toggle.onclick = () => {
    ...
    pauseBack.hidden = !(open && panel.closest("#pause-settings"));
    pauseBack.textContent = "‹  " + (toggle.dataset.caption || "Language");
    if (panel.closest("#pause-settings")) document.getElementById("dialog").scrollTop = 0;
    position();
    if (open)
      (
