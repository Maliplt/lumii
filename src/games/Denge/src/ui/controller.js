...
if (typeof document !== "undefined")
  (() => {
    ...
    const campaigns = DengeCampaignData.loadCampaigns(storage);
    const hintWallet = DengeCampaignData.loadHintWallet(storage);
    const earnedStars = () => DengeCampaignData.totalStars(campaigns);
    function syncHints() {
      hintWallet.refresh();
    ...
    const levelMap = createDengeLevelMap({
      $,
      campaigns,
      getLevel: () => level,
      getState: () => state,
      ...
      t,
      levelName,
      availableStars: () => Math.max(0, earnedStars() - hintWallet.spent),
      reduceMotion,
    });
    function buildBoard() {
    ...
    function startGame(number = campaigns[level].next) {
      ...
      if (number === "1" && !tutorial.hasSeen()) {
        tutorial.begin(number);
        return;
      }
      let board;
      try {
    ...
    function closeDialog(resume = true) {
      document.querySelector(".topbar").append($("settings-controls"));
      $("pause-settings").hidden = true;
      $("pause-language-back").hidden = true;
      $("language-panel").hidden = true;
      $("language").setAttribute("aria-expanded", "false");
      document.body.append($("language-panel"));
      $("dialog").close();
      if (dialogKind === "tutorial") tutorial.cancelPending();
      dialogKind = "";
    ...
    const tutorial = createDengeTutorial({
      $,
      t,
      storage,
      audio,
    ...
    function renderDialog() {
      ...
      $("pause-settings").hidden = !inPause;
      if (inPause) {
        const captions = { language: "pauseLanguage", theme: "pauseTheme", help: "help", sound: "pauseSound" };
        for (const [id, label] of Object.entries(captions)) {
          $(id).dataset.caption = t(label);
        }
        $("pause-settings").append($("settings-controls"));
        $("pause-settings").append($("language-panel"));
      } else {
        document.querySelector(".topbar").append($("settings-controls"));
        $("pause-language-back").hidden = true;
        $("language-panel").hidden = true;
      ...
      if (kind === "pause") {
        ...
        $("dialog-actions").lastElementChild.insertAdjacentHTML("afterbegin", '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 5 11 7-11 7Z"/></svg>');
        dialogButton(
          "levelMap",
          () => {
            dialogKind = "leave";
        ...
        $("dialog-actions").lastElementChild.insertAdjacentHTML("afterbegin", '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z"/></svg>');
        return;
      }
      if (kind === "leave") {
    ...
    $("help").onclick = () => {
      if ($("dialog").open && dialogKind === "pause") {
        dialogKind = "help";
        renderDialog();
