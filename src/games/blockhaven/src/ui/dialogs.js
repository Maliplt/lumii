...
(function (B) {
  ...
  function open({ title, body, actions = [], dismissible = true, onClose, className = "" }) {
    ...
    if (dismissible) {
      panel.append(h("button", { type: "button", class: "icon-btn icon-btn--small dialog__close", label: "common.close", onclick: () => dialog.close("dismiss") }, icon("close", { size: 20 })));
      backdrop.addEventListener("pointerdown", (event) => event.target === backdrop && dialog.close("dismiss"));
    }
    root.append(backdrop);
    stack.push(dialog);
    requestAnimationFrame(() => (panel.querySelector(".btn--primary") || panel.querySelector("button"))?.focus({ preventScroll: true, focusVisible: false }));
    return dialog;
  }
  ...
  function confirm({ title, text }) {
    return new Promise((resolve) => {
      open({
        ...
        body: h("p", {}, text),
        actions: [
          { label: B.t("common.no"), kind: "ghost", onClick: () => resolve(false) },
          { label: B.t("common.yes"), kind: "primary", onClick: () => resolve(true) },
        ],
