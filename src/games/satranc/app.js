...
(() => {
  ...
  function renderSetup() {
    ...
    $("opponents").innerHTML = opponents
      .map(
        (o, i) =>
          `<button class="opponent ${level === i + 1 ? "selected" : ""}" data-level="${i + 1}" aria-pressed="${level === i + 1}"><span class="portrait">${JourneyArt.avatar(i, i)}</span><span><strong>${o.name}</strong><small>${t(o.style)} · ${o.code}</small><small>+${o.reward} XP${data.wins[i + 1] ? " · " + data.wins[i + 1] + " ✓" : ""}</small></span>${stars(o.strength)}</button>`,
      )
      .join("");
    $("local-art").innerHTML =
      ChessArt.piece("K") +
  ...
  function renderPlayers() {
    for (const [position, side] of [
      ...
      const profile = ChessProgress.data.profile;
  ...
  function requestAI() {
    ...
    const fallback = () => {
      ...
      setTimeout(() => {
        if (id !== workId) return;
        accept(ChessAI.choose(state, session.level, 200));
      }, 80);
    };
    try {
      worker = new Worker("worker.js");
      worker.onmessage = ({ data: reply }) => {
        if (reply.move) accept(reply.move);
      };
      worker.onerror = fallback;
      worker.postMessage({ id, state, level: session.level });
    } catch {
      fallback();
    }
  }
    ...
    const score =
      result.winner === "w"
        ...
        : result.winner === "b"
          ? "0 : 1"
          : "½ : ½";
    openDialog(`<div class="result result-${outcome}">
