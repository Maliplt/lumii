...
if (typeof module !== "undefined" && module.exports)
  module.exports = require("../../core/rules/engine.js");
if (typeof document !== "undefined")
  (() => {
    ...
    const {
      ...
      currentAngle = (rotation, at) => animatedAngle(rotation, at, reduce());
    ...
    function labels() {
      ...
      if (modalController.isOpen()) modalController.render();
    ...
    const renderResultView = PrizmaScreens.createResultRenderer({
      element: $,
      translate: t,
      formatTime: time,
      getLanguage: () => lang,
    ...
    function renderResult() {
      renderResultView(lastResult);
    }
    const modalController = PrizmaDialog.create({
      element: $,
      createElement: (tag) => document.createElement(tag),
      translate: t,
      icon: svg,
      getGameState: () => state,
      canOpen: () => !busy && state !== "celebrating",
      pauseGame: () => {
        change("paused");
        persistSession();
      },
      resumeGame: () => change("playing"),
      startTutorial: (returnState) => {
        tutorialReturn = returnState;
    ...
    const openModal = (kind) => modalController.open(kind);
    const closeModal = (resume = true) => modalController.close(resume);
    async function goMap(center = true) {
      ...
      tutorial = false;
      hinted = -1;
      if (center) mapScreen.center(progress.next);
      renderMap();
      await change("map");
      renderMap();
      audio.play("open");
    }
    const mapScreen = PrizmaScreens.createMapRenderer({
      element: $,
      createElement: (tag) => document.createElement(tag),
      translate: t,
      icon: svg,
      circuit: Circuit,
      themeMarkup: (markup) => PrizmaTheme.markup(markup),
      getProgress: () => progress,
      launch: (number) => launch(number),
      canShift: () => !busy && state === "map",
      reducedMotion: reduce,
      playSound: (name) => audio.play(name),
    });
    const renderMap = () => mapScreen.render();
    const shiftMap = (delta) => mapScreen.shift(delta);
    ...
    $("map-current").onclick = () => {
      mapScreen.center(progress.next);
      renderMap();
    };
