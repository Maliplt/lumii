...
(function (K) {
  ...
  function open({ title, body, actions = [], dismissible = true, onClose, className = "" }) {
        ...
        h(
          ...
          actions.map((action) =>
            h(
              ...
              {
                type: "button",
                class: `tab tab--dialog tab--${{ primary: "coral", ghost: "paper" }[action.kind] || action.kind || "paper"}`,
                onclick: () => {
    ...
    if (dismissible) {
      panel.append(h("button", { type: "button", class: "dot dot--paper dot--small dialog__close", label: "common.close", onclick: () => dialog.close("dismiss") }, icon("close", { size: 20 })));
      backdrop.addEventListener("pointerdown", (event) => event.target === backdrop && dialog.close("dismiss"));
    }
    root.append(backdrop);
    stack.push(dialog);
    requestAnimationFrame(() => (panel.querySelector(".tab--coral") || panel.querySelector("button"))?.focus({ preventScroll: true, focusVisible: false }));
    return dialog;
  }
  ...
  function confirm({ title, text }) {
    return new Promise((resolve) => {
      open({
        ...
        body: h("p", {}, text),
        actions: [
          { label: K.t("common.no"), kind: "ghost", onClick: () => resolve(false) },
          { label: K.t("common.yes"), kind: "primary", onClick: () => resolve(true) },
        ],
