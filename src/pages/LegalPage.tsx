...
export default function LegalPage() {
  ...
  const active: SectionKey = section && section in SECTIONS ? (section as SectionKey) : 'gizlilik'
      ...
      <div className="legal-container">
        ...
        <Nav className="legal-tabs" activeKey={active}>
          {ORDER.map((key) => {
            ...
            return (
              <Nav.Item key={key} eventKey={key} as={Link} to={`/legal/${key}`} className="legal-tab">
                <Icon size={16} />
                <span>{label}</span>
              </Nav.Item>
            )
